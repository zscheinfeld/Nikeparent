...
show() {
  blendMode(ADD); // Optional: Try different blend modes for vibrant results
  noFill();
  noStroke();
  ...
  for (let i = 1; i < this.tail.length; i++) {
    ...
    // Gradient effect: interpolate color along the tail
    let tA = map(i - 1, 0, this.tail.length, 1, 0);
    let tB = map(i, 0, this.tail.length, 1, 0);

    let baseColor = color(this.color[0], this.color[1], this.color[2], 255);
    let fadedColor = color(this.color[0], this.color[1], this.color[2], 0);
    let colorA = lerpColor(baseColor, fadedColor, tA);
    let colorB = lerpColor(baseColor, fadedColor, tB);
